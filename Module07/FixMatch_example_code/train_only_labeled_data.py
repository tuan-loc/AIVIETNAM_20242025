...
def main():
    ...
    parser.add_argument('--gpu-id', default='3', type=int, help='id(s) for CUDA_VISIBLE_DEVICES')
    ...
    parser.add_argument("--opt_level", type=str, default="O1", help="apex AMP optimization level selected in ['O0', 'O1', 'O2', and 'O3']."
    ...
    print("  Task = {args.dataset}@{args.num_labeled}")
    print("  Num Epochs = {args.epochs}")
    print("  Batch size per GPU = {args.batch_size}")
    print("  Total train batch size = {args.batch_size*args.world_size}")
    print("  Total optimization steps = {args.total_steps}")
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader, model, optimizer, ema_model, scheduler):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            # ground-truth of labeled data
            targets_x = targets_x.to(args.device)

            # weakly augmented version of unlabeled data
            inputs_u_w = inputs_u_w.to(args.device)
            # strongly augmented version of unlabeled data
            inputs_u_s = inputs_u_s.to(args.device)

            # extract prediction of labeled data
            logits_labeled_data = model(inputs_x)

            # extract prediction of weakly labeled data
            logits_unlabeled_data_weak = model(inputs_u_w)
            # extract prediction of strongly labeled data
            logits_unlabeled_data_strong = model(inputs_u_s)
